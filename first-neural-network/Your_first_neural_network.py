...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            hidden_inputs = np.dot(X,self.weights_input_to_hidden) # signals into hidden layer shape is (2,)
            hidden_outputs = self.activation_function(hidden_inputs) # signals from hidden layer shape is (2,)

            # TODO: Output layer - Replace these values with your calculations.
            final_inputs = np.dot(hidden_outputs,self.weights_hidden_to_output) # signals into final output layer shape is (1,)
            final_outputs = final_inputs # signals from final output layer shape is (1,)
            
            #### Implement the backward pass here ####
            ### Backward pass ###

            # TODO: Output error - Replace this value with your calculations.
            error = y - final_outputs # Output layer error is the difference between desired target and actual output. shape is (1,)
            
            # TODO: Backpropagated error terms - Replace these values with your calculations.
            output_error_term = error   
            
            # TODO: Calculate the hidden layer's contribution to the error
            hidden_error = output_error_term * self.weights_hidden_to_output # checked

            hidden_error_term = hidden_error * ((hidden_outputs * (1 - hidden_outputs)).reshape(self.hidden_nodes,1)) # shap (2,1)

            # Weight step (input to hidden)
            delta_weights_i_h +=  hidden_error_term.reshape(self.hidden_nodes,) * X[:,None]
